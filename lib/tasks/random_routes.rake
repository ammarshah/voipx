  ...
  task export: :environment do
    ...
    Route.all.limit(500).each do |route|
      begin
        ...
        export_routes_sheet.insert_row(export_routes_sheet.last_row_index + 1, [route.id, route.breakout.destination, route.breakout.code.to_i, route.price, route.quality_type, route.purchase_type, user.id, user.name])
        puts "--- INSERTED! ---"
        counter += 1
      rescue Exception => e
